...
var WpFileUpload = function (containerId) {
    ...
    // 设置初始化函数
    wfu.getFileItemResultModelArrive = WpFileUploadViewsModel.getFileItemResultModelArrive;
    // 初始化选择文件
    wfu.initWithSelectFile = WpFileUploadEvent.initWithSelectFile;
    // 初始化清除文件
    wfu.initWithCleanFile = WpFileUploadEvent.initWithCleanFile;
    // 初始化文件上传
    wfu.initWithUpload = WpFileUploadEvent.initWithUpload;
    // 初始化对象文件
    wfu.initFileList = WpFileUploadFileList.initFileList;
    // 清除
    wfu.cleanFileEvent = WpFileUploadEvent.cleanFileEvent;
    return wfu;
};
/**
 * 文件上传参数工具
...
var WpFileUploadEvent = {
    ...
    "initWithSelectFile" : function(wfu){
        var uploadId = wfu.uploadId;
        var selectFileBtObj = $("#"+uploadId+" .uploadBts .selectFileBt");
    ...
    "initWithUpload" : function(wfu){
        ...
        if(!wfu.isHiddenUploadBt){
            // 上传文件按钮
            var uploadFileBt = $("#"+uploadId+" .uploadBts .uploadFileBt");
            uploadFileBt.off();
            uploadFileBt.on("click",function(){
                WpFileUploadTools.uploadFileEvent(wfu);
            });
            // 上传按钮图标
            var uploadFileBtIcon = $("#"+uploadId+" .uploadBts .uploadFileBt i");
...
var WpFileUploadViewsModel = {
    ...
    "getHeadButtonsView" : function(wfu){
        // 选择文件的按钮标题
        var selectFileButtonTitle = '选择文件';
        var btsStr = '';
     ...
     * @returns {string} 返回回显文件的模版
     */
    "getFileItemResultModelArrive" : function (fileType, fileId, fileName, isImg, fileUrl, deleteFile, downloadFile,meta_url) {
        //默认显示类型
        var showTypeStr = '<div class="fileType">'+fileType+'</div> <i class="iconfont icon-wenjian"></i>';
        //是否是文件
        if (isImg) {
        ...
        if (downloadFile) {
            modelStr += '<div class="down">';
            modelStr += '<i class="iconfont icon-xiazai" mid="'+fileId+'" onclick="downloadfile(\''+fileId+'\',\''+meta_url+'\',\''+fileName+'\')"></i>';
            modelStr += '</div>';
        }
        if (deleteFile) {
            modelStr += '<div class="status">';
            modelStr += '<i class="iconfont icon-shanchu" mid="'+fileId+'" onclick="deletefile(\''+fileId+'\')"></i>';
            modelStr += '</div>';
        }
        modelStr += '<div class="fileName">';
        modelStr += fileName;
...
var WpFileUploadTools = {
    ...
    "disableFileSelect":function(wfu){
        var uploadId = wfu.uploadId;
        var selectFileBt = $("#"+uploadId+" .uploadBts .selectFileBt");
