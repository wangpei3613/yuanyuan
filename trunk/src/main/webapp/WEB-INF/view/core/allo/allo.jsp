...
<head>
	...
	$(function(){
		grid = $('#sen-datagrid').grid({
			url:'${path }/sen/core/allo/queryAlloInfo',
			sortName:'addtime',
		    sortOrder:'desc',
			method:'get',
			columns:[[
	...
	function openAdd(){
		layer.win({
		  title: '新增分配模型',
		  area: ['400px', '300px'],
		  content: '${path}/sen/core/allo/toAlloEdit',
		  btn:['保存','关闭'],
	...
	function openEdit(){
		if(grid.grid('selectOne')){
		     layer.win({
				  title: '修改分配模型',
				  area: ['400px', '300px'],
				  content: '${path}/sen/core/allo/toAlloEdit?id='+grid.grid('getSelected').id,
				  btn:['保存','关闭'],
				  yes:function(index, layero){
					  var win = layero.find('iframe')[0].contentWindow,
					  	  form = win.$('form');
					  if(form.form('validate')){
						  sen.ajax({
							  url:'/sen/core/allo/editAlloInfo',
							  data:form.serializeObject(),
							  loaddingEle:layero,
							  success:function(){
								  top.layer.close(index);
