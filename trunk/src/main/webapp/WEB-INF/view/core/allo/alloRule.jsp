...
<html>
<head>
	<title>分配模型规则配置</title>
	<%@ include file="../../index/lib.jsp" %>
	<script type="text/javascript">
	var grid;
	$(function(){
		grid = $('#sen-datagrid').grid({
			url:'${path }/sen/core/allo/queryAlloRule',
			sortName:'sort',
		    sortOrder:'asc',
		    queryParams:{alloid:'${param.id}'},  
		    fitColumns:false,
		    method:'get',
			columns:[[
				...
				{field:'sort',title:'优先级',width:60,sortable:true,align:'center'},
				{field:'isend',title:'是否终止规则',width:100,sortable:true,align:'center',dictType:'SEN_IF'},
	...
	function openAdd(){
		layer.win({
		  title: '新增分配模型规则',
		  area: ['800px', '500px'],
		  content: '${path}/sen/core/allo/toAlloRuleEdit?alloid=${param.id}',
		  btn:['保存','关闭'],
	...
	function openEdit(){
		if(grid.grid('selectOne')){
		     layer.win({
				  title: '修改分配模型规则',
				  area: ['800px', '500px'],
				  content: '${path}/sen/core/allo/toAlloRuleEdit?id='+grid.grid('getSelected').id,
				  btn:['保存','关闭'],
				  yes:function(index, layero){
					  var win = layero.find('iframe')[0].contentWindow,
					  	  form = win.$('form');
					  if(form.form('validate')){
						  sen.ajax({
							  url:'/sen/core/allo/editAlloRule',
							  data:form.serializeObject(),
							  loaddingEle:layero,
							  success:function(){
								  top.layer.close(index);
...
<body class="easyui-layout sen-crud-page">
    ...
    <div class="sen-crud-page-center" data-options="region:'center',border:false">
    		<c:forEach items="${authFunctionCode }" var="auth">
    		  <c:if test="${auth == 'alloManage:alloRule' }">
